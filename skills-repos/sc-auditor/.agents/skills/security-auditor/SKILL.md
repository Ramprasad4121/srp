...
2. **Run Slither**: Call `run-slither` with `{rootDir: "<current>"}` where `<current>` is the current directory. Store the returned findings, limited to the scope defined in step 1.

3. **Run Aderyn**: Call `run-aderyn` with `{rootDir: "<current>"}`  where `<current>` is the current directory. Store the returned findings, limited to the scope defined in step 1.

4. **Load Checklist**: Call `get_checklist` with no arguments (or `{}`) to load the full Cyfrin audit checklist.
...
2. **Load Relevant Checklist Items**: Call `get_checklist` with `{category: "<relevant_category>"}` to get checklist items for the function's domain (e.g., "Reentrancy", "Access Control", "Oracle").

3. **Search for Similar Patterns**: Call `search_findings` with `{query: "<pattern_description>"}` to find real-world examples of similar vulnerabilities on Solodit. Use optional parameters `severity`, `tags`, and `limit` to narrow results when appropriate.
...
- Search for similar findings that were invalidated: call `search_findings` with relevant queries
...
Call `search_findings` with `{query: "<vulnerability_description>"}` to find similar confirmed findings on Solodit. Include matching results as additional evidence sources.
