...
export class OrchestrationEngine {
  ...
  async execute(mode: ExecutionMode, agentIds: string[], input: unknown): Promise<unknown[]> {
    if (mode === "parallel") {
      const results = await Promise.all(agentIds.map((id) => this.runWithRetry(id, input)));
