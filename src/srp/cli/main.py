...
def cmd_init(args):
    ...
    from srp.core.policy import PolicyConfig
...
def cmd_analyze(args):
    ...
    from srp.core.intent import ExecutionIntent
    from srp.core.policy import ERC8004PolicyClient
    from srp.core.budget import X402BudgetEngine
    from srp.core.agent import OpenClawWorker
    from srp.core.trace import ReasoningTrace
    from srp.core.pipeline import MultiPassReasoningPipeline, PASS_SEQUENCE
...
def cmd_status(args):
    ...
    print(f"  Model         : claude-sonnet-4-6 (Anthropic)")
...
def cmd_dev(args):
    """
    I1: srp dev — Launch the SRP Developer Mode server on port 7338.
    Provides real-time NatSpec, invariants, test generation, gas, and upgrade safety
    as you write Solidity code.
    """
    import subprocess
    import socket
    import sys
    ...
    port = getattr(args, 'port', 7338)
    project_root = str(Path.cwd())

    print("""
╔══════════════════════════════════════════════════════════╗
║  SRP Developer Mode — AI-powered dev feedback            ║
║  NatSpec · Invariants · Tests · Gas · Upgrade Safety     ║
╚══════════════════════════════════════════════════════════╝
""")
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        if s.connect_ex(('127.0.0.1', port)) == 0:
            print(f"[SRP Dev] ❌ Port {port} already in use.")
            print(f"[SRP Dev] Kill: lsof -ti:{port} | xargs kill -9")
            sys.exit(1)

    # Check for dev_server.py
    dev_server_path = Path(__file__).resolve().parent.parent / "dev_server.py"
    if not dev_server_path.exists():
        # Fall back to server.py with DEV_MODE env
        dev_server_path = Path(__file__).resolve().parent.parent / "server.py"
        if not dev_server_path.exists():
            print("[SRP Dev] ❌ dev_server.py not found")
            sys.exit(1)

    env = {
        **os.environ,
        "SRP_DEV_MODE": "true",
        "SRP_PROJECT_ROOT": project_root,
        "SRP_DEV_PORT": str(port),
    }

    print(f"[SRP Dev] ✅ Launching dev server at http://localhost:{port}")
    print(f"[SRP Dev] Project: {project_root}")
    print(f"[SRP Dev] Watching: *.sol files")
    print(f"[SRP Dev] Press Ctrl+C to stop\n")
...
Examples:
  ...
  srp verify --trace srp-traces/abc123.json
    ...
    # audit (shortcut for server launch)
    subparsers.add_parser("audit", help="Launch audit server and open dashboard")

    # dev (I1)
    dev_parser = subparsers.add_parser("dev", help="Launch SRP developer mode (port 7338)")
    dev_parser.add_argument("--port", type=int, default=7338, help="Port for dev server (default: 7338)")
