...
def audit(target, no_browser, port):
    ...
    uvicorn.run("srp.server.server:app", host="0.0.0.0", port=port, reload=False, log_level="warning")
...
@cli.command()
@click.option("--port", default=7338, help="Port to run on (default: 7338)")
def dev(port):
    """Start the SRP Developer mode (AI-powered real-time Solidity feedback)."""
    import threading
    console.print(f"[cyan]Starting SRP Dev Server on port {port}...[/cyan]")
    def open_browser():
        time.sleep(2)
        webbrowser.open(f"http://localhost:{port}")
    threading.Thread(target=open_browser, daemon=True).start()
    ...
    os.environ["SRP_DEV_PORT"] = str(port)
    uvicorn.run("srp.server.dev_server:app", host="0.0.0.0", port=port, reload=False, log_level="warning")
...
def status():
    ...
    # Check LLM Keys
    has_llm = bool(os.environ.get("NVIDIA_API_KEY"))
    if has_llm:
        console.print("[green]✅ LLM API Key: set (NVIDIA NIM)[/green]")
    else:
        console.print("[red]❌ LLM API Key: not set (NVIDIA_API_KEY required)[/red]")
