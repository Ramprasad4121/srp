...
class TraceAgent(BaseAgent):
    ...
    def _repo_from_skill_path(path: str) -> str | None:
        ...
        if normalized.startswith("src/srp/skills/audit-firm-1/"):
            return "audit-firm-1"
        if normalized.startswith("src/srp/skills/quillai/"):
            return "quillai"
        if normalized.startswith("src/srp/skills/trailofbits/"):
            return "trailofbits"
        if normalized.startswith("src/srp/skills/scv-scan/"):
            return "scv-scan"
        if normalized.startswith("src/srp/skills/cyfrin/"):
            return "cyfrin"
        return None
