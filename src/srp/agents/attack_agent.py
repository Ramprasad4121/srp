...
from srp.agents.v2.audit_swarm import AuditSwarmV2
...
"""
        ...
        exploit_result, ghost_result, zero_result = await asyncio.gather(
            self._run_courtroom_debate_pass(contract_map, entry_points, hypothesis_result),
    ...
    async def _run_courtroom_debate_pass(
        ...
        # Format hypotheses for the Swarm
        hypo_strings = []
        for h in top_hypotheses:
            title = h.get("title", "Untitled")
            desc = h.get("description", "")
            hypo_strings.append(f"Title: {title}\nDescription: {desc}")
            
        # Combine the source code for the swarm
        code = "\\n".join(str(v) for v in contract_map.values() if isinstance(v, str))

        swarm = AuditSwarmV2(run_id=uuid4().hex[:8])
        result = await swarm.run({
            "code": code,
            "hypotheses": hypo_strings
        })
        
        vulnerabilities = self._ensure_list(result.get("findings", []))
        self.log_step(
            f"{pass_name}_pass_completed", {"vulnerability_count": len(vulnerabilities)}
        )
        return {"vulnerabilities": vulnerabilities, "attack_summary": f"Courtroom Debate finalized {len(vulnerabilities)} findings."}
