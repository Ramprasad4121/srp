"""
SlashingPropagationChecker — Validates slashing flows to delegators.

Checks: slash handling fairness, partial slash accounting, front-running.
"""
...
class SlashingPropagationChecker(BaseAgent):
    """Audits how slashing impacts all stakers."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="SlashingPropagationChecker",
            role="Staking specialist — audits slashing propagation",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("slashing_audit_started", {})
...
You are SLASHING PROPAGATION CHECKER. Focus on:
1. Slashing applies to all stakers fairly (no escape)
2. Partial slash math (proportional reduction)
3. Front-running slash via early withdrawal
4. Withdrawal delay vs slash timing
5. Slashing notification

Return JSON: {{"vulnerabilities": [{{"id": "STAKE-SLASH-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("slashing_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"STAKE-SLASH-{idx:03d}")),
                "title": ensure_finding_title(f, "Slashing Propagation Bug", "incorrect slash distribution"),
                "severity": sev,
