"""
WithdrawalQueueAuditor — Validates withdrawal queue ordering and processing.

Checks: FIFO enforcement, queue manipulation, griefing.
"""
...
class WithdrawalQueueAuditor(BaseAgent):
    """Audits withdrawal queue mechanics."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="WithdrawalQueueAuditor",
            role="Staking specialist — audits withdrawal queue ordering",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("withdrawal_queue_audit_started", {})
...
You are WITHDRAWAL QUEUE AUDITOR. Focus on:
1. FIFO ordering enforcement
2. Queue position manipulation
3. Griefing (spam queue to block others)
4. Claim timing (fixed epochs vs continuous)
5. Partial withdrawal handling

Return JSON: {{"vulnerabilities": [{{"id": "STAKE-QUEUE-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("withdrawal_queue_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"STAKE-QUEUE-{idx:03d}")),
                "title": ensure_finding_title(f, "Withdrawal Queue Bug", "queue manipulation or locked withdrawals"),
                "severity": sev,
