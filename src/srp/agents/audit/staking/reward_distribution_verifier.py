"""
RewardDistributionVerifier — Validates reward distribution fairness.

Checks: rounding precision, dust accumulation, manipulation.
"""
...
class RewardDistributionVerifier(BaseAgent):
    """Audits reward distribution math."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="RewardDistributionVerifier",
            role="Staking specialist — validates reward distribution",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("reward_distribution_audit_started", {})
...
You are REWARD DISTRIBUTION VERIFIER. Focus on:
1. Precision loss (reward per share calculation)
2. Dust accumulation/unclaimed rewards
3. First-last depositor fairness
4. Rounding direction (must favor protocol)
5. Manipulation via donation

Return JSON: {{"vulnerabilities": [{{"id": "STAKE-REWARD-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("reward_distribution_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"STAKE-REWARD-{idx:03d}")),
                "title": ensure_finding_title(f, "Reward Distribution Bug", "incorrect reward accounting"),
                "severity": sev,
