"""
MarkPriceHunter — Finds oracle and mark price manipulation vectors.

Checks: TWAP manipulation, spot price reliance, staleness.
"""
...
class MarkPriceHunter(BaseAgent):
    """Hunts for mark price manipulation vulnerabilities."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="MarkPriceHunter",
            role="Perpetuals specialist — hunts mark price manipulation",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("mark_price_audit_started", {})
...
You are MARK PRICE HUNTER. Focus on:
1. TWAP vs spot price vulnerability
2. Oracle staleness checks
3. Single-block manipulation
4. Circuit breakers for large deviations
5. Index vs mark divergence

Return JSON: {{"vulnerabilities": [{{"id": "PERP-MARK-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("mark_price_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"PERP-MARK-{idx:03d}")),
                "title": ensure_finding_title(f, "Mark Price Manipulation", "liquidation price distortion"),
                "severity": sev,
