"""
FundingRateAuditor — Validates funding rate calculation and manipulation resistance.

Checks: funding calculation, payment timing, rate caps.
"""
...
class FundingRateAuditor(BaseAgent):
    """Audits funding rate mechanics."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="FundingRateAuditor",
            role="Perpetuals specialist — audits funding rate mechanics",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("funding_rate_audit_started", {})
...
You are FUNDING RATE AUDITOR. Focus on:
1. Funding rate calculation (mark vs index)
2. Payment timing and settlement
3. Rate caps and circuit breakers
4. Manipulation resistance
5. Time-weighted average precision

Return JSON: {{"vulnerabilities": [{{"id": "PERP-FUND-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("funding_rate_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"PERP-FUND-{idx:03d}")),
                "title": ensure_finding_title(f, "Funding Rate Bug", "incorrect funding settlement"),
                "severity": sev,
