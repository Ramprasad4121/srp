"""
ProposalOrderingAnalyzer — Analyzes front-running of governance execution.

Checks: proposal sandwich, execution reordering.
"""
...
class ProposalOrderingAnalyzer(BaseAgent):
    """Analyzes proposal ordering and execution vulnerabilities."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="ProposalOrderingAnalyzer",
            role="Governance specialist — analyzes proposal ordering",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("proposal_ordering_audit_started", {})
...
You are PROPOSAL ORDERING ANALYZER. Focus on:
1. Front-running of governance execution (MEV)
2. Proposal sandwich attacks
3. Execution reordering within same block
4. Race conditions between queue and execute
5. Proposal ID manipulation

Return JSON: {{"vulnerabilities": [{{"id": "GOV-ORDER-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("proposal_ordering_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"GOV-ORDER-{idx:03d}")),
                "title": ensure_finding_title(f, "Proposal Ordering Bug", "out-of-order governance execution"),
                "severity": sev,
