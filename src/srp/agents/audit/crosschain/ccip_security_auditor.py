"""
CCIPSecurityAuditor — Chainlink CCIP specific security checks.

Checks: receiver validation, fee token handling, message ordering.
"""
...
class CCIPSecurityAuditor(BaseAgent):
    """Audits Chainlink CCIP implementation."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="CCIPSecurityAuditor",
            role="Cross-chain specialist — audits Chainlink CCIP",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("ccip_audit_started", {})
...
You are CCIP SECURITY AUDITOR. Focus on:
1. Receiver validation (onlyRouter modifier)
2. Fee token validation and handling
3. ccipReceive permission checks
4. MessageId uniqueness and replay
5. Token transfer limits and rate limiting
6. ccipSend return value handling

Return JSON: {{"vulnerabilities": [{{"id": "XCHAIN-CCIP-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("ccip_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"XCHAIN-CCIP-{idx:03d}")),
                "title": ensure_finding_title(f, "CCIP Security Bug", "unauthorized CCIP message execution"),
                "severity": sev,
