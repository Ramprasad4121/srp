"""
MessageOrderingChecker — Validates cross-chain message ordering.

Checks: out-of-order execution, replay across chains.
"""
...
class MessageOrderingChecker(BaseAgent):
    """Checks message ordering across chains."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="MessageOrderingChecker",
            role="Cross-chain specialist — checks message ordering",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("message_ordering_audit_started", {})
...
You are MESSAGE ORDERING CHECKER. Focus on:
1. Out-of-order message processing
2. Replay across different chains
3. Nonce per source enforcement
4. Sequence gaps and handling
5. Message expiry and timeout

Return JSON: {{"vulnerabilities": [{{"id": "XCHAIN-ORDER-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("message_ordering_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"XCHAIN-ORDER-{idx:03d}")),
                "title": ensure_finding_title(f, "Message Ordering Bug", "out-of-order cross-chain execution"),
                "severity": sev,
