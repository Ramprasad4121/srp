"""
BridgeAssetReconciler — Validates asset accounting across chains.

Checks: supply consistency, mint/burn accounting.
"""
...
class BridgeAssetReconciler(BaseAgent):
    """Reconciles bridge asset accounting."""

    def __init__(self, model: str = "meta/llama-3.1-405b-instruct") -> None:
        super().__init__(
            name="BridgeAssetReconciler",
            role="Cross-chain specialist — reconciles bridge assets",
    ...
    async def run(self, context: dict) -> dict:
        self.log_step("bridge_asset_reconcile_audit_started", {})
...
You are BRIDGE ASSET RECONCILER. Focus on:
1. Cross-chain supply consistency
2. Mint/burn event accounting
3. TotalSupply invariant across chains
4. Wrapped token backing verification
5. Liquidity discrepancy detection

Return JSON: {{"vulnerabilities": [{{"id": "XCHAIN-ASSET-001", "title": "...", "severity": "high|medium|low", "description": "...", "contract": "...", "vuln_code": "...", "fix_code": "...", "exploit_code": "..."}}]}}"""
        ...
        self.log_step("bridge_asset_reconcile_audit_completed", {"count": len(vulns)})
    ...
    def _normalize(self, findings: Any) -> list[dict]:
        ...
        for idx, f in enumerate(findings, 1):
            ...
            if sev not in {"critical", "high", "medium", "low"}:
                sev = "medium"
            normalized.append({
                "id": str(f.get("id", f"XCHAIN-ASSET-{idx:03d}")),
                "title": ensure_finding_title(f, "Asset Reconciliation Bug", "cross-chain asset imbalance"),
                "severity": sev,
