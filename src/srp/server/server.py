...
from srp.core.orchestrator import SRPOrchestrator
from srp.core.project import SRPProject
from srp.core.skill_loader import SkillLoader, SkillNotFoundError
...
from srp.server.gateway import gateway_router
...
async def get_emergency():
    ...
    if not project.initialized:
        return {}
    from srp.core.audit_progress import AuditProgress
...
async def run_audit(contract_code: str, description: str = "", api_key: str | None = None):
    ...
    try:
        ...
        # Persist audit artifacts into the target project's .srp history.
        if trace_id:
            try:
                project = get_project()
                if project.initialized:
                    project.save_audit(trace_id, result)
                    project.save_trace(trace_id, trace)
                    if report_md:
                        project.save_report(trace_id, report_md)
            except Exception as exc:
                audit_state["logs"].append(f"⚠️ Failed to persist audit artifacts: {exc}")
...
# Resolve project root: __file__ is at src/srp/server/server.py
def find_static_dir():
    # 1. Look for 'static' relative to this file (src/srp/server/server.py -> ../../../static)
    base = Path(__file__).resolve().parent.parent.parent.parent
    candidate1 = base / "static"
    if candidate1.exists():
        return str(candidate1)
    
    # 2. Look in CWD
    candidate2 = Path.cwd() / "static"
    if candidate2.exists():
        return str(candidate2)
    
    # 3. Look in package root (site-packages/srp/static)
    candidate3 = Path(__file__).resolve().parent.parent / "static"
...
if static_dir:
    print(f"  ✅ UI Static Files found: {static_dir}")
    app.mount("/static", StaticFiles(directory=static_dir), name="static")
else:
    print("  ❌ UI Static Files: NOT FOUND — check your project root for 'static/'")
...
if __name__ == "__main__":
    uvicorn.run(app, host="0.0.0.0", port=7337, reload=False)
